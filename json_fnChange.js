...
function validate(task) {
    ...
    if (!validators.isMissing(changeInfoReasons)) {
    ...
    for (field in ["name", "vatin", "host", "dns"]) {
...
function execute(task) {
    ...
    Fptr.setParam(Fptr.LIBFPTR_PARAM_FN_OPERATION_TYPE, Fptr.LIBFPTR_FNOP_CHANGE_FN);
    ...
    if (ffdVersion === Fptr.LIBFPTR_FFD_1_0_5) {
        ...
        // Корректируем коды причин перерегистрации
        changeInfoReasons |= 0x01;
