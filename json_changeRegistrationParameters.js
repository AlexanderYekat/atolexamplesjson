...
function validate(task) {
    ...
    // reason проверим при выполнении задания - важна версия ФФД
    var changeInfoReasons = validators.mustArrayOrMissing(task.changeInfoReasons, "changeInfoReasons");
    if (changeInfoReasons) {
        for (var i = 0; i < changeInfoReasons.length; i++) {
            validators.mustNumber(changeInfoReasons[i], "changeInfoReasons[" + i + "]");
        }
    }
    ...
    ["name", "vatin", "host", "dns"].forEach(function (item, index) {
        validators.mustStringOrMissing(ofd[item], "ofd." + item);
    });
...
function execute(task) {
    ...
    // В открытой смене или на незарегистрированной ККТ сразу выдаем ошибку
    Fptr.setParam(Fptr.LIBFPTR_PARAM_DATA_TYPE, Fptr.LIBFPTR_DT_STATUS);
    if (Fptr.queryData() < 0) {
        return Fptr.error();
    }
    if (!Fptr.getParamBool(Fptr.LIBFPTR_PARAM_FISCAL)) {
        return Fptr.result(Fptr.LIBFPTR_ERROR_FISCAL_PRINTER_NOT_ACTIVATED);
    }
    ...
    // Запоминаем флаг автоматического режима - по нему будем отключать запись некоторых параметров дальше
    Fptr.setParam(Fptr.LIBFPTR_PARAM_FN_DATA_TYPE, Fptr.LIBFPTR_FNDT_TAG_VALUE);
    Fptr.setParam(Fptr.LIBFPTR_PARAM_TAG_NUMBER, 1002);
    if (Fptr.fnQueryData() < 0) {
        return Fptr.error();
    }
    var offlineMode = Fptr.getParamBool(Fptr.LIBFPTR_PARAM_TAG_VALUE);
    if (!validators.isMissing(task.device.offlineMode)) {
        offlineMode = task.device.offlineMode;
    }
    ...
    Fptr.setParam(Fptr.LIBFPTR_PARAM_FN_OPERATION_TYPE, Fptr.LIBFPTR_FNOP_CHANGE_PARAMETERS);
    ...
    if (ffdVersion === Fptr.LIBFPTR_FFD_1_0_5) {
        switch (task.reason) {
            ...
            case "attributesChange":
                ...
                break;
            case "settingsChange":
                Fptr.setParam(1101, 4);
                break;
            default:
                return Fptr.result(Fptr.LIBFPTR_ERROR_RECEIPT_PARSE_ERROR, "Некорректное значение поля \"reason\" (" + task.reason + ")");
        }
    } else {
        var changeInfoReasons = 0;
        if (!validators.isMissing(task.changeInfoReasons)) {
            for (var i = 0; i < task.changeInfoReasons.length; i++) {
                changeInfoReasons |= (1 << task.changeInfoReasons[i]);
            }
        }

        Fptr.setParam(1205, ToUInt32(changeInfoReasons));
    }
